...
export function generateStaticParams() {
  const posts = getFinanzPostsByTopic('finances', true);
...
export async function generateMetadata({
  ...
  const post = getFinanzPostBySlugAndTopic(slug, 'finances');
  if (!post) return {};
...
export default async function FinancesPostPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const schema = blogPostingSchema(post, 'finances');
